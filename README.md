...
| **윤후성** | • 여기내용 추가좀 <br> 가능하면 밑에 상세내용이랑 동일하게 <br> • VisionModel, LLM Model , BackEnd Server CI/CD 배포환경 <br>  • Problem Detection Domain 설계 및 구현 |
| **성기현** | • VisionModel Finetunning API 연결 |

### 주요 기능
| 번호 | 기능                                       | 설명                                                                                                                                                                                                                                                       |
| -- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | **철도 영상 기반 <br> 분석 통합 파이프라인**         | 1) VisionModel → LLM → Backend으로 이어지는 데이터 통합 처리 흐름 설계 <br> 2) Vision Model, LLM 모델 결과(JSON, Image, Zip)를 수신하여 내부 도메인 모델로 변환 <br> 3) Scheduler Worker 패턴을 활용한 VisionModel, LLM 호출 병렬화 <br> 4) 분석 단계(Status) 기반 처리 흐름 제어 및 실패 시 재시도·예외 처리 로직 설계 |
| 2  | **S3 + CloudFront 기반 <br> 파일 관리 시스템** | 1) 대용량 영상·이미지·JSON 파일을 AWS S3 저장 <br> 2) CloudFront CDN 기반 고정 URL 구조로 분산 서빙 아키텍처 구성 <br> 3) Backend는 파일 메타데이터만 관리하도록 책임 분리 <br> 4) CDN 캐싱 전략 적용으로 서버 트래픽 분산 및 병목 제거                                                
| 3  | **세션 기반 <br> Authentication**    | 1) Spring Security 기반 서버 세션 인증 구조 구현 <br> 2) 로그인/로그아웃 및 인증 사용자 정보 조회 API 구성 <br> 3) CloudFornt + S3 프론트 배포환경에 따른 세션 문제 해결                                                       |
| 4  | **유지보수(규정) 문서 CRUD 및 <br> RAG 데이터 연동**   | 1) 유지보수 기준 문서 등록·수정·삭제·조회 API 설계 <br> 2) 운영 단계에서 규정 변경 시 즉시 반영 가능한 구조 설계 <br> 3) RAG 파이프라인에서 활용 가능한 데이터 제공 계층 구현                                                                             |
| 5  | **엔지니어 피드백 API**     | 1) 탐지 결과에 대한 수정·보정 정보 입력 API 구현 <br> 2) Vision Model 파인튜닝 API 연결 <br> 3) Vision 모델 고도화를 위한 학습 데이터 레이어 구성                               |
...
2. [Session 로그인 FrontEnd CI/CD 배포 환경](#Session-로그인-FrontEnd-CI/CD-배포-환경)
